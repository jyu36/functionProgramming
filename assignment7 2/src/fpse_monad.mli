...
module type S = sig
  ...
  (** [list_fold_m a ~init ~f] folds left-to-right through the list, binding on the accumulated value
      at each step. *)

  val list_map_m : 'a list -> f:('a -> 'b m) -> 'b list m
  (** Like [List.map], but applies [f] in monadic context and collects results. Items in the list are
      mapped with [f] from left to right, so monadic effects from the start of the list happen first. *)
