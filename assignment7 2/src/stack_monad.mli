...
module Make (S : sig type t end) : sig
  include module type of State_monad.Make (struct type t = S.t list end)
  ...
  *)
  ...
  (** [pop] is a monadic value whose underlying data is the top value of the stack, 
      and the new state no longer has the top value. This may throw an exception when
      the stack is already empty. *)

  val run : 'a m -> 'a
  (** [run x] takes [x] out of monad-land by providing an initial empty stack and throwing 
      away the final stack. Recall that ['a] is the underlying data type. From this signature,
